...
export class ZbtService {
  ...
  private async sendToUser(message: Message, state: 'accepted' | 'denied', deniedReason?: string) {
    ...
    if (state === 'accepted') {
      return dm.send(
        'Ваша заявка на закрытое бэта-тестирование была **одобрена** ✅.\n' +
        'Вам была выдана роль, а так же Вы добавлены в вайт-лист и можете заходить на сервер.\n' +
        `Optifine 1.20.1 - ${inlineCode('play.arcania.world')}\n` +
        `\nЖелаем приятной игры!`,
      );
    } else if (state === 'denied') {
      return dm.send(
        'Ваша заявка на закрытое бэта-тестирование была **отклонена** 🛑.\n' +
        (deniedReason ? `Причина: ${inlineCode(deniedReason)}` : 'Причина не указана.') +
        `\nВы можете повторно подать заявку <t:${~~((Date.now() + hour(12)) / 1000)}:R>`,
      );
    }
    return;
  }
